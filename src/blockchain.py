...
def compute_upd_2(weights, base, updates, lrate):
    """
    纯平均聚合 (FedAvg)。忽略外部传入的 weights，强行设为 1/N。
    lrate 相当于聚合时的全局学习率，这里默认传入1也问题不大。
    """
    # 1. 强制将所有客户端权重设为平均
    ...
    for client in updates.keys():
        ...
        for param_name in ['w1', 'w2', 'wo', 'b1', 'b2', 'bo']:
            upd[param_name] += (lrate * weights[client]) * model[param_name]
...
def compute_global_model(base_block, updates, lrate, aggregator="FedAvg"):
    ...
    for client, update in updates.items():
        reset()  # 清理图或会话
        dataset = dataext.load_data("data/mnist.d")  # 注意这里是否有用
